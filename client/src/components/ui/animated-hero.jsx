...
export const ParticleHero = ({
  title = "GAMEGEAR",
  subtitle = "Cyber Setup",
  description = "Experience the mesmerizing dance of performance and RGB lights.",
  primaryButton,
  secondaryButton,
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < totalParticles; i++) {
      ...
      // Color intensity based on distance (Gaming gear: Cyan and Purple/Red colors!)
      const hue = Math.round(180 + (col / rows) * 120);
  ...
  useEffect(() => {
    const checkReset = setInterval(() => {
      if (!isAutoMode && Date.now() - lastMouseMoveRef.current >= 4000) {
        setIsAutoMode(true);
        setIsStaticAnimation(false);
  ...
  return (
    <section 
      className={`relative w-full min-h-[75vh] bg-black overflow-hidden flex items-center ${className}`}
    ...
    >
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center px-6 py-12">
        {children ? (
          ...
          <div className="text-center max-w-6xl mx-auto">
            {/* Main Title */}
            <div className="mb-8">
              <h1 className="text-6xl md:text-[8rem] font-black tracking-tighter leading-[0.8] mb-6">
                <span className="bg-gradient-to-r from-cyan-400 via-primary to-purple-500 bg-clip-text text-transparent drop-shadow-2xl">
                  {title}
                </span>
              </h1>
              ...
              <div className="space-y-3">
                <h2 className="text-lg md:text-2xl font-thin text-cyan-200/90 tracking-[0.2em] uppercase">
                  {subtitle}
                </h2>
            ...
            {description && (
              <div className="mb-8">
                <p className="text-sm md:text-base text-cyan-100/60 font-light max-w-2xl mx-auto leading-relaxed">
                  {description}
                </p>
              </div>
            )}
            ...
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                {primaryButton && (
                  <button 
                    ...
                    className="group relative px-8 py-3 bg-transparent border border-primary/40 hover:border-primary text-cyan-200 hover:text-white font-semibold tracking-wider uppercase transition-all duration-500 overflow-hidden rounded-lg text-sm"
                  >
                ...
                {secondaryButton && (
                  <button 
                    ...
                    className="px-8 py-3 border-2 border-white/20 hover:border-primary text-white hover:text-primary font-semibold rounded-lg transition-all duration-300 backdrop-blur-sm text-sm"
                  >
              ...
              {/* Interactive hint */}
              {interactiveHint && (
                <div className="flex items-center justify-center gap-4 text-primary/40 text-[10px] uppercase tracking-[0.3em] mt-4">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-purple-600/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[120vh] h-[120vh] bg-gradient-radial from-primary/5 to-transparent rounded-full"></div>
      </div>
    </section>
  );
};
